...
export default function CreateRequestForm({ categories, onSubmit, student }) {
  ...
  return (
    <form onSubmit={formik.handleSubmit} noValidate className="font-sans">
      <div className="mb-4">
        <label htmlFor="title" className="block text-[10px] font-semibold uppercase tracking-wider text-neutral-500 mb-1">Title</label>
        <input type="text" placeholder="e.g. Fan not working" {...field('title')} />
        {formik.touched.title && formik.errors.title && (
          <div className="text-[10px] text-red-600 mt-1">{formik.errors.title}</div>
        )}
      </div>

      <div className="mb-4">
        <label htmlFor="description" className="block text-[10px] font-semibold uppercase tracking-wider text-neutral-500 mb-1">Description</label>
        <textarea
          id="description"
          rows={3}
          ...
          className={`w-full bg-white border rounded px-3 py-2 text-xs text-neutral-900 placeholder-neutral-400 outline-none transition duration-150 focus:border-neutral-500 focus:ring-1 focus:ring-neutral-500 resize-none ${
            formik.touched.description && formik.errors.description ? 'border-neutral-900' : 'border-neutral-200'
          }`}
          {...formik.getFieldProps('description')}
        />
        {formik.touched.description && formik.errors.description && (
          <div className="text-[10px] text-red-600 mt-1">{formik.errors.description}</div>
        )}
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-4 mb-4">
        <div className="md:col-span-5">
          ...
          {formik.touched.category && formik.errors.category && (
            <div className="text-[10px] text-red-600 mt-1">{formik.errors.category}</div>
          )}
        </div>

        <div className="md:col-span-3">
          <label htmlFor="roomNo" className="block text-[10px] font-semibold uppercase tracking-wider text-neutral-500 mb-1">Room No.</label>
          <input
            type="text"
            readOnly
          ...
          {formik.touched.roomNo && formik.errors.roomNo && (
            <div className="text-[10px] text-red-600 mt-1">{formik.errors.roomNo}</div>
          )}
        </div>
        ...
        <div className="md:col-span-4">
          ...
          {formik.touched.priority && formik.errors.priority && (
            <div className="text-[10px] text-red-600 mt-1">{formik.errors.priority}</div>
          )}
        </div>
      </div>
